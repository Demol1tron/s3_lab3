...
OperationRes T::search(const std::string &value) const
{
    ...
    return searchRecursive(root_, value) ? OperationRes::SUCCESS : OperationRes::NOT_FOUND;
}
...
void T::printTreeVisual(const TNode *node, const std::string &prefix, bool isLeft) const
{
    if (!node) return;
...
void T::print() const
{
    std::cout << "=== Дерево '" << name_ << "' (размер: " << size_ << ") ===" << std::endl;
