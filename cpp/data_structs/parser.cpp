...
ParsedCommand parseQuery(const std::string &query)
{
    ...
    if (iss >> token) {
        cmd.operation = token;
        // std::transform(cmd.operation.begin(), cmd.operation.end(), cmd.operation.begin(), ::toupper);
    }
...
OperationRes executeCommand(Storage *storage, const ParsedCommand &cmd, std::string &result)
{
    ...
    if (cmd.operation == "PRINT") {
        if (cmd.structure_name.empty() || cmd.structure_name == "ALL") {
            ...
            while (current) {
                switch (current->type) {
                    case StructType::ARRAY: static_cast<M*>(current->structure)->print(); break;
                    case StructType::SINGLY_LIST: static_cast<F*>(current->structure)->print(); break;
                    case StructType::DOUBLY_LIST: static_cast<L*>(current->structure)->printForward(); break;
                    case StructType::STACK: static_cast<S*>(current->structure)->print(); break;
                    case StructType::QUEUE: static_cast<Q*>(current->structure)->print(); break;
                    case StructType::TREE: static_cast<T*>(current->structure)->print(); break;
                    case StructType::SET: static_cast<Set*>(current->structure)->print(); break;
                    default: std::cout << "Неизвестный тип структуры" << std::endl; break;
                }
                std::cout << std::endl;
            ...
            switch (namedStruct->type) {
                case StructType::ARRAY: static_cast<M*>(namedStruct->structure)->print(); break;
                case StructType::SINGLY_LIST: static_cast<F*>(namedStruct->structure)->print(); break;
                case StructType::DOUBLY_LIST: static_cast<L*>(namedStruct->structure)->printForward(); break;
                case StructType::STACK: static_cast<S*>(namedStruct->structure)->print(); break;
                case StructType::QUEUE: static_cast<Q*>(namedStruct->structure)->print(); break;
                case StructType::TREE: static_cast<T*>(namedStruct->structure)->print(); break;
                case StructType::SET: static_cast<Set*>(namedStruct->structure)->print(); break;
                default: return OperationRes::ERROR;
            }
            return OperationRes::SUCCESS;
        }
    }
    ...
    if (namedStruct && namedStruct->type == StructType::ARRAY) {
        ...
        if (cmd.operation == "MPUSH" && cmd.arguments.size() >= 1) {
            if (cmd.arguments.size() == 1) return arr->pushBack(cmd.arguments[0]);
            else { size_t index = std::stoul(cmd.arguments[0]); return arr->pushAt(index, cmd.arguments[1]); }
        } else if (cmd.operation == "MGET" && cmd.arguments.size() == 1) {
            size_t index = std::stoul(cmd.arguments[0]); return arr->get(index, result);
        } else if (cmd.operation == "MDEL" && cmd.arguments.size() == 1) {
            size_t index = std::stoul(cmd.arguments[0]); return arr->delAt(index);
        } else if (cmd.operation == "MREPLACE" && cmd.arguments.size() == 2) {
            size_t index = std::stoul(cmd.arguments[0]); return arr->replace(index, cmd.arguments[1]);
        } else if (cmd.operation == "MFIND" && cmd.arguments.size() == 1) {
            int index = arr->find(cmd.arguments[0]);
            if (index != -1) { result = std::to_string(index); return OperationRes::SUCCESS; }
            return OperationRes::NOT_FOUND;
        } else if (cmd.operation == "MSIZE") {
            result = std::to_string(arr->getSize()); return OperationRes::SUCCESS;
        }
    }
        ...
        if (cmd.operation == "FPUSH" && cmd.arguments.size() >= 1) {
            if (cmd.arguments.size() == 1) return list->pushBack(cmd.arguments[0]);
            else if (cmd.arguments.size() == 2) {
                std::string position = toLower(cmd.arguments[0]);
                if (position == "front") return list->pushFront(cmd.arguments[1]);
                else if (position == "back") return list->pushBack(cmd.arguments[1]);
            }
            ...
            std::string position = toLower(cmd.arguments[0]);
            if (position == "front") return list->delFront();
            else if (position == "back") return list->delBack();
            else if (position == "value" && cmd.arguments.size() == 2) return list->delByValue(cmd.arguments[1]);
            ...
            return list->delBefore(target);
             ...
             if (cmd.arguments[0] == "size") { result = std::to_string(list->getLength()); return OperationRes::SUCCESS; }
        } else if (cmd.operation == "FFIND" && cmd.arguments.size() == 1) {
            if (list->find(cmd.arguments[0])) { result = "НАЙДЕН"; return OperationRes::SUCCESS; }
            return OperationRes::NOT_FOUND;
        }
    }
        ...
        if (cmd.operation == "SPUSH" && cmd.arguments.size() == 1) return stack->push(cmd.arguments[0]);
        else if (cmd.operation == "SPOP") return stack->pop(result);
        else if (cmd.operation == "SSIZE") { result = std::to_string(stack->getSize()); return OperationRes::SUCCESS; }
    }

    else if (namedStruct && namedStruct->type == StructType::QUEUE) {
        Q *queue = static_cast<Q*>(namedStruct->structure);
        if (cmd.operation == "QPUSH" && cmd.arguments.size() == 1) return queue->enqueue(cmd.arguments[0]);
        else if (cmd.operation == "QPOP") return queue->pop(result);
        else if (cmd.operation == "QSIZE") { result = std::to_string(queue->getSize()); return OperationRes::SUCCESS; }
    }

    else if (namedStruct && namedStruct->type == StructType::TREE) {
        T *tree = static_cast<T*>(namedStruct->structure);
        if (cmd.operation == "TINSERT" && cmd.arguments.size() == 1) return tree->insert(cmd.arguments[0]);
        else if (cmd.operation == "TSEARCH" && cmd.arguments.size() == 1) {
             result = (tree->search(cmd.arguments[0]) == OperationRes::SUCCESS) ? "ДА" : "НЕТ"; return OperationRes::SUCCESS;
        } else if (cmd.operation == "TISFULL") { result = tree->isFull() ? "ДА" : "НЕТ"; return OperationRes::SUCCESS; }
        else if (cmd.operation == "TSIZE") { result = std::to_string(tree->getSize()); return OperationRes::SUCCESS; }
    }

    else if (namedStruct && namedStruct->type == StructType::SET) {
        Set *set = static_cast<Set*>(namedStruct->structure);
        if (cmd.operation == "SETADD" && cmd.arguments.size() == 1) return set->add(cmd.arguments[0]);
        else if (cmd.operation == "SETDEL" && cmd.arguments.size() == 1) return set->remove(cmd.arguments[0]);
        else if (cmd.operation == "SETSIZE") { result = std::to_string(set->getSize()); return OperationRes::SUCCESS; }
        else if (cmd.operation == "SETAT" && cmd.arguments.size() == 1) {
            if (set->contains(cmd.arguments[0])) { result = "TRUE"; return OperationRes::SUCCESS; }
            return OperationRes::NOT_FOUND;
        }
    }
    ...
    if (cmd.operation == "DELETE") 
        ...
        result = namedStruct ? "ДА" : "НЕТ"; return OperationRes::SUCCESS;
    }
    return OperationRes::ERROR;
}